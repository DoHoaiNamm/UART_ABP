`include "define.h"
module uart_apb_tx(/*AUTOARG*/
   // Outputs
   tx_busy, uart_sout, rd_tx_fifo, sout, 
   tx_shift_empty, tx_enable,
   // Inputs
   pclk, resetn, uart_enable, tx_fifo_empty, 
...
if(uart_enable)   begin
	casex(state)  // synopsys parallel_case 
		...
		TX_WAIT: 	begin 
			if(brg_clken) next_state = TX_LOAD;
		...
		TX_DATA: 	begin 
			...
			rd_tx_fifo = 1'b0;
			sout = tx_shift[0];
			casex({tx_end_frame,half_stop,u_bc,tx_enable}) // synopsys parallel_case 
				4'b1000: next_state = TX_IDLE;
				4'b101x: next_state = TX_BC;
				4'b1001: next_state = TX_LOAD;
				4'b11xx: next_state = TX_HALFSTOP;
		...
		TX_HALFSTOP: begin
			...
			rd_tx_fifo = 1'b0;
			sout = 1'b1;
			casex({tx_mid_bit,u_bc,tx_enable}) // synopsys parallel_case 
				3'b11x: next_state = TX_IDLE;
				3'b100: next_state = TX_BC;
				3'b101: next_state = TX_LOAD;
...
always @(posedge pclk) begin
	if(resetn == 1'b0) begin
		...
		TX_LOAD: begin	
			...
			tx_count_bit  <=`DELAY  4'd0;
			if(brg_clken)tx_count_clk  <=`DELAY  tx_count_clk +1'd1;
		end
		TX_STARTBIT: begin
			if(brg_clken)tx_count_clk  <=`DELAY  tx_count_clk +1'd1;
			if(tx_end_bit) tx_count_bit  <=`DELAY  tx_count_bit + 1'd1;
		end
		TX_DATA: begin
			if(brg_clken) tx_count_clk  <=`DELAY  tx_count_clk +1'd1;
			if(tx_end_bit) begin
				tx_shift  <=`DELAY  {1'b1,tx_shift[8:1]};
				tx_count_bit  <=`DELAY  tx_count_bit + 1'd1;
			end
		end
		TX_HALFSTOP: begin
			if (tx_mid_bit) tx_count_clk  <=`DELAY  4'd0;
			else if(brg_clken) tx_count_clk  <=`DELAY  tx_count_clk +1'd1;
		end
	endcase
end
//
